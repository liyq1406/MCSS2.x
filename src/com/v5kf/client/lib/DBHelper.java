...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TBL_FMT = " create table if not exists %s " +
    		"(_id integer primary key autoincrement, session_start integer, message_id text, hit integer,state integer,direction integer ,json_content text, create_time integer)";
    // SQLiteDatabase实例
	...
	public boolean insert(V5Message message, boolean force) {
		...
		if (message == null || message.getDirection() == V5MessageDefine.MSG_DIR_RELATIVE_QUES ||
				(message.getMessage_type() == V5MessageDefine.MSG_TYPE_CONTROL)) {
			// 不保存[相关问题]和非[转人工客服]的控制消息 -> [修改]过滤全部控制消息 // && ((V5ControlMessage)message).getCode() != 1
			Logger.w("v5client", "DbHelper insert message null or type not accept");
			return false;
		}
		...
		if (!force) {
			...
			if (hasMessageId(message.getMessage_id())) {
				Logger.w("v5client", "DbHelper already has message:" + message.getMessage_id());
		...
		Logger.w("DBHelper", "DbHelper insert message:" + json_content);
    ...
    public void querySession(List<V5Message> msgs, int order) {
    	...
    	while (cur.moveToNext()) {
			...
			try {
				...
				V5Message message = V5MessageManager.receiveMessage(jsonMsg);
				message.setHit(hit);
	...
	public boolean querySession(List<V5Message> msgs, int offset, int size) {
    	...
    	while (cur.moveToNext()) {
			...
			try {
				Logger.d("DBHelper", "[querySession] json_content:" + content);
				JSONObject jsonMsg = new JSONObject(content);
				V5Message message = V5MessageManager.receiveMessage(jsonMsg);
				message.setHit(hit);
				message.setState(state);
