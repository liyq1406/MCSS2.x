 ...
 *
 */
public class HistoryVisitorFragment extends BaseFragment implements OnRefreshListener {
     ...
     * number.
     */
    public static HistoryVisitorFragment newInstance(int sectionNumber) {
    	HistoryVisitorFragment fragment = new HistoryVisitorFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	mView = inflater.inflate(R.layout.fragment_contact_customer, container, false);
...
//			getLastMonthCustomer();
    ...
    private void initView() {
    	mRecyclerAdapter = new HistoryVisitorAdapter(mRecycleBeans, mActivity);
    	mRecyclerView = (RecyclerView) mView.findViewById(R.id.id_recycle_view);
        ...
        /* 空白按钮 */
        mView.findViewById(R.id.layout_container_tv).setOnClickListener(new OnClickListener() {
        		...
        		if (newState == RecyclerView.SCROLL_STATE_IDLE && lastVisibleItem + 1 == mRecyclerAdapter.getItemCount()
                    ...
                    if (isLoadingMore) {
                    	...
                    	if (mHasMore || mRecyclerAdapter.getItemCount() < mAppInfo.getVisitorMap().size()) {
                    		...
                    		isLoadingMore = false;
                    		mHandler.sendEmptyMessage(HDL_STOP_LOAD);
                    		mActivity.ShowShortToast(R.string.no_more);
                    	}
                    	Logger.i(TAG, "上拉加载 ...");
                    }
                }
    ...
    private void checkListEmpty() {
    	if (null == mView) { // [修改]null object错误，可能mView为空导致
    		mView = getView();
    		if (getView() == null) {
    			return;
    		}
    	...
    	if (mRecycleBeans.size() == 0) {
    		...
    		mView.findViewById(R.id.layout_container_empty).setVisibility(View.VISIBLE);
		} else {
			mSwipeRefresh.setVisibility(View.VISIBLE);
			mView.findViewById(R.id.layout_container_empty).setVisibility(View.GONE);
		}
    }
...
//    	}
	...
	private void getMonthCustomer() {
		...
		try {
			TicketRequest tReq = (TicketRequest) RequestManager.getRequest(QAODefine.O_TYPE_WTICKET, mActivity);
	...
	public void onFragmentSelected() {
		Logger.d(TAG, "onFragmentSelected - HistoryVisitor");
	...
	private void setLoadMoreVisible(boolean visible) {
		if (visible) {
			mView.findViewById(R.id.load_more_layout).setVisibility(View.VISIBLE);
			mRecyclerView.scrollToPosition(mRecyclerAdapter.getItemCount() - 1);
		} else {
			mView.findViewById(R.id.load_more_layout).setVisibility(View.GONE);
//			mRecyclerView.scrollToPosition(mRecyclerAdapter.getItemCount() - 1);
		}
	}
